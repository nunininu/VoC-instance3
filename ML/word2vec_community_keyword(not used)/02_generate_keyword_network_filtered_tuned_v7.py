...
# DOMINANT_CENTRALITY_PORTION = 0.25
# MIN_COUNT = 5

MIN_COUNT = 7                           # 단발 단어 제거
TOP_N_NODES = 80                        # 상위 중심 노드만 제한
EDGE_LIMIT = 5000                       # 복잡도 제한 강화
THRESHOLD_START = 0.99                  # 높은 임계값부터 탐색
THRESHOLD_END = 0.94                    # 낮은 임계값까지 탐색
THRESHOLD_STEP = -0.005                 # 임계값 탐색 간격 축소
DOMINANT_CENTRALITY_THRESHOLD = 0.95    # 중심성 상위 노드 비율 제한
DOMINANT_CENTRALITY_PORTION = 0.2       # 중심성 상위 노드 비율 제한
...
# 색상 팔레트 지정 (최대 10개 커뮤니티)
COMMUNITY_COLORS = [
    "#FF4B4B", "#4B7BFF", "#00CC99", "#FFBB33", "#CC66FF",
    "#FF6699", "#33CCCC", "#AAAA00", "#FF9966", "#66FF66"
]

# 중심성 변환
centrality_scaled_log = {
    k: np.clip(np.log1p(v), 0.05, 1.0)
    for k, v in centrality_scaled.items()
}
...
with open("force_directed_keyword_graph_filtered_tuned_v7.html", "w", encoding="utf-8") as f:
    f.write(html)

print("✅ force_directed_keyword_graph_filtered_tuned_v7.html 생성 완료")
