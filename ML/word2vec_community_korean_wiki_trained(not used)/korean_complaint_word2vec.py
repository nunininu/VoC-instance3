...
class WikiComplaintWord2VecSystem:
    ...
    def download_and_process_wikipedia(self):
        """위키피디아 데이터 다운로드 및 전처리"""
        print("=== 위키피디아 데이터 다운로드 ===")
        
        # 위키피디아 덤프 다운로드 (주석처리 - 실제 환경에서 실행)
        # !wget https://dumps.wikimedia.org/kowiki/latest/kowiki-latest-pages-articles.xml.bz2
        # !python -m wikiextractor.WikiExtractor kowiki-latest-pages-articles.xml.bz2
        
        print("위키피디아 데이터 처리를 위해서는 다음 단계가 필요합니다:")
    ...
    def preprocess_wiki_text(self, wiki_file="wiki_merged.txt"):
        ...
        print(f"총 {len(lines)}개의 줄을 처리합니다...")
        
        # 형태소 분석
        result = []
        for line in tqdm(lines, desc="형태소 분석"):
            # 빈 문자열이 아니고, XML 태그가 아닌 경우에만 처리
            if line and not line.startswith('<') and not line.startswith('</'):
                try:
                    morphs = self.mecab.morphs(line)
                    if len(morphs) > 1:  # 최소 2개 이상의 형태소가 있는 경우만
                        result.append(morphs)
                except:
                    continue
        
        print(f"전처리 완료: {len(result)}개의 문장")
        return result
...
# 메인 실행 함수
def main_wiki_complaint_system():
    """전체 시스템 실행"""
...
if __name__ == "__main__":
    # 전체 시스템 실행
    system = main_wiki_complaint_system()
